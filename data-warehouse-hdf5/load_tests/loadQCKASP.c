/* loadKASP.c, DEM 14apr16, from loadSEED.c, DEM 6apr2016, from ...
   from example code h5_crtdat.c etc.  */

/* KASP format(?), tab-separated, 3 lines before the header, beginning
   with "S/N".  Four leading columns before allele calls.  Coding like
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    printf("Example: %s /home/matthews/Data/Maize/QC_KASP/Plates_358-367_2nd_DH_pops_QC_data_x_103_SNPs-defatted.csv /shared_data/HDF5/Maize/QC_KASP_Plates_358-367.h5 QC_KASP\n", argv[0]);
    return 0;
  }
  ...
  /* Read the 4th line of the input file to get the marker names. */
  ...
  while ((token = strtok(NULL, "\t\n\r"))) {
    /* Omit the four leading columns before the data (S/N, DNA ID, Plate, Well etc.). */
    if (outndx > 2) {
      /* Read the rest of the input line into dset_data[]. */
      markernames[outndx - 3] = strdup(token);
      /* printf ("row %i, name = %s\n", outndx, markernames[outndx - 10]); */
    }
    outndx++;
  }
  int MarkerCount = outndx - 3;
  fclose(infile);
  ...
  rownum = 0;
  infile = fopen(infilename, "r");
  /* Skip four lines of header. */
  for (r = 0; r < 4; r++)
    fgets (row, 1000000, infile);
  while (fgets (row, 1000000, infile)) {
    token = strtok(row, "\t");
    /* printf("first token, matrix read:%s\n", token); */
      outndx = 0;
      /* Strip \n and possible \r from the end of the last token. */
      while ((token = strtok(NULL, "\t\n\r"))) {
	/* Omit the thirteen leading columns before the data (S/N, DNA ID, Plate, Well etc.). */
	if (outndx > 2) {
	  /* Read the rest of the input line into dset_data[]. */
	  dset_data[outndx - 3] = iupac(token);
	}
	++outndx;
      }
