/* load55K, DEM 14apr16, from loadBeadXpress, DEM 13apr16, ... 
   ... from example code h5_crtdat.c etc.  */

/* Axiom(?) format, tab-separated, 1 line before the header, beginning
   with "header".  One leading column before allele calls, the
   marker name.  Coding single nucleotide, "?" or empty. */
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    printf("Example: %s /home/matthews/Data/Maize/55K_chip/DTMA_50Ksnps_samplenamed.txt /shared_data/HDF5/Maize/DTMA_50Ksnps_101023.h5 DTMA\n", argv[0]);
    return 0;
  }
  ...
  if (strcmp(token, "header") != 0) {
    printf("Didn't find header row, exiting.\n");
    return(0);
  }
...
/* Translate allele scores to one-character IUPAC "ambiguity" code. */
char iupac(char *alleles) {
  ...
  if (strcmp(alleles, "") == 0)
    return('N');
  /* Warning, loss of data! Currently storing heterozygotes as missing data. */
  if (strcmp(alleles, "H") == 0)
    return('N');
