...
curl -i -H "Accept: application/json" -H "Content-Type: application/json"  -H "Cache-Control: no-cache, no-store, must-revalidate" -d "{\"pingRequests\":[\"Ping request from curl 1\",\"Ping request from curl 2\"],\"pingResponses\":[]}" localhost:8080/gobii-web/load/ping
